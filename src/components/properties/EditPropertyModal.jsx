...
export function EditPropertyModal({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (validateForm()) {
      // Delete removed images from server first
      for (const publicId of removedImageIds) {
        try {
          await propertiesApi.removeImage(property._id, publicId)
        } catch (err) {
          console.error('Failed to delete image:', err)
          toast.error('Failed to delete one or more images')
        }
      }
      onSubmit(property._id, formData)
    }
  }
